...
5. 中国大陆网络的 Python 包默认使用清华 PyPI 镜像；失败后只换中科大镜像重试一次。
6. 安装结束后运行本地 `scripts/doctor.js`，用命令退出码和一次最终检查验收，不截断关键输出，不重复执行无必要的 `pip list`。

## 国内镜像与网络判断

- Python 包首选清华：`https://pypi.tuna.tsinghua.edu.cn/simple`。
- Python 包备用中科大：`https://pypi.mirrors.ustc.edu.cn/simple`。
...
- 镜像出现 `502`、`403`、TLS 或代理错误时，只换另一个国内镜像重试一次。
