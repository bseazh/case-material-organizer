...
```bash
npx github:bseazh/case-material-organizer#v0.3.0 install
```
...
- 生成 `整理结果/案件材料汇总.xlsx`、简明 `index.txt`、基础资料索引；
...
空分类目录仍会保留，并注明“本次未发现相关材料”。`index.txt` 只显示文件总数、成功解析数、未解析数和目录树，不向普通用户展示技术映射。
