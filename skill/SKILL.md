---
name: case-material-organizer
description: 整理用户指定的案件材料文件夹：提取可读内容和日期，按时间链路分类、重命名、复制归档，生成案件总览 Excel、文件索引 TXT，并按用户确认可选生成 HTML/PNG/PDF 时间轴。音视频不转写，仅检查并匹配逐字稿。
---

# 案件材料整理

本 Skill 处理用户指定的“现成文件夹”，不负责解压，不默认扫描文件夹外内容。

## 必须遵守

- 原始文件夹只读：不移动、不覆盖、不删除、不直接改名；结果复制到新的结果文件夹。
- 不补造事实；把材料记载、当事人陈述、系统记录和待核事项分开。
- 图片、扫描件、PDF、DOCX、XLSX、CSV、TXT、JSON 可提取或 OCR；保留原件。
- 音频和视频当前版本只登记和归档，不播放、不转写、不推断内容，也不阻塞其他材料整理。
- 仅创建五个材料分类目录：`001 主体信息`、`002 基础资料`、`003 委托材料`、`004 类案及法律检索`、`005 法律文书`；所有用户成果集中到 `整理结果`，机器文件放入其下的 `技术资料`。
- 归档、重命名、生成时间轴前必须按交互规则取得用户确认。

## 工作阶段

1. 读取 `references/interaction.md`，让用户选择或确认输入文件夹。
2. 读取 `references/execution.md`，运行或参照 `scripts/inventory.py` 递归清点文件，记录原路径、扩展名、大小、哈希、可读性和候选日期。
3. 读取 `references/extraction.md` 提取文字、主体、地点、日期、金额和候选事实；音视频仅登记为“当前版本不处理”。
4. 依次读取 `references/entity-resolution.md`、`references/dedup-version.md`、`references/event-model.md`，先统一主体并识别重复/版本，再合并事件。
5. 读取 `references/classification.md` 与 `references/naming.md`，生成五类目录、标准文件名和冲突清单预览。
6. 读取 `references/directory-tree.md`，生成 `归档目录预览.md`；在对话中直接展示完整目录树和 A/B/C 选项。未确认前不得执行归档。
7. 确认后读取 `references/output-schema.md`，执行归档并在 `整理结果` 中生成 Excel、简明索引及目录报告，再在对话中展示实际目录树。
8. Excel 是面向普通用户的阅读界面：第一张表必须是基于合并事件的“案件概览”，不显示内部编号、哈希、重复组、版本组或机器路径。
9. 完成后询问是否生成时间轴；选择生成时才读取 `references/timeline.md` 并运行 `scripts/build_timeline.py`。
10. 交付前读取 `references/qa.md` 并逐项核验；任何关键项失败都不得声称完成。

## 默认交互

每个确认点只给 A/B/C 选项，避免让用户重新描述全部需求。解析失败、日期冲突、主体不确定时暂停确认，不猜测。

## 输出最低要求

- `整理结果/案件材料汇总.xlsx`：只展示案件概览、材料名称、当事人、时间轴和待补材料等用户可读信息；内部追踪数据仅保留在技术 JSON 中。
- `整理结果/index.txt`：只展示文件总数、成功解析数、未解析数和实际目录树。
- `002 基础资料/index.md`：按日期排序的材料索引。
- `整理结果/归档结果目录.md`：简明统计、实际目录树和下一步交互，不显示内部编号或文件映射。
- `整理结果/技术资料/归档方案_已执行.json`：机器追溯数据，普通用户无需查看。
- 五个材料分类目录：空目录也保留，并写明“本次未发现相关材料”。

## 渐进式披露

只在对应阶段读取所需参考文件，不要一次性加载全部规则。若用户要求改变目录、命名、媒体或时间轴规则，先更新本次任务的确认方案，再继续执行。
